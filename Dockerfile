...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 CMD .venv/bin/python -c "import httpx,sys; r = httpx.get('http://localhost:8000/health-check'); sys.exit(0) if r.status_code==200 else sys.exit(1)"
